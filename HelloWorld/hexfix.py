...
def convert(s):
    ...
    for c in s:
            ...
            if ord(0) <= ascii <= ord('9'):  # It's a decimal number, and return it as decimal:

                total = total + ascii - ord('0')
            elif ord('A') <= ascii <= ord('F'):  # It's a hex number between 10 and 15, convert and return:
                total = total + ascii - ord('A') + 10
            elif ord('a') <= ascii <= ord(
            'f'):  # Check if they used lower case: It's a hex number between 10 and 15, convert and return:
